...
def main_menu():
    ...
    history_button = button.Button(304, 250, history_image, 0.5)
    exit_button = button.Button(304, 375, exit_image, 0.5)
    botplay_button = button.Button(550, 175, botplay_image, 0.5)
    humanplay_button = button.Button(550, 325, humanplay_image, 0.5)
    back_button = button.Button(20, 10, back_image, 0.35)
    blackside_button = button.Button(304, 375, blackside_image, 0.5)
    whiteside_button = button.Button(304, 125, whiteside_image,0.5)
    ...
    while run:
        ...
        if screen_state == "play_menu":
            draw_text("Choose your game mode", pygame.font.SysFont("arialblack", 30), TEXT_COlOR, 100, 250)
            if botplay_button.draw_self(screen):
                screen_state = "bot_menu"
        ...
        if screen_state == "bot_menu":
            draw_text("Choose your side", pygame.font.SysFont("arialblack", 20), TEXT_COlOR, 284, 60)
            if blackside_button.draw_self(screen):
                player1 = False
                player2 = True
                run = False
